  ...
  return `
    <div class="p-card ${wholesale ? "wholesale" : "retail"} ${outOfStock ? "out-of-stock" : ""}" data-id="${p.id}">
      ...
      <div class="p-card__img">
        <img src="${ssImg(p)}" alt="${p.name}" loading="lazy" onclick="location.href='/templates/product-detail.html?id=${p.id}'">
...
function ssQuickAdd(id) {
  ...
  if (!p) { location.href = `/templates/product-detail.html?id=${id}`; return; }
  ...
  function renderResults(products, query, total) {
    ...
    boxEl.querySelectorAll(".sug-item").forEach(item => {
      item.addEventListener("click", () => {
        location.href = `/templates/product-detail.html?id=${item.dataset.id}`;
      });
    });
...
function ssFlashSaleCard(fs) {
  ...
  return `
    <div class="p-card fs-card ${soldOut ? "out-of-stock" : ""} ${locked ? "fs-card--locked" : ""}" data-fsid="${fsId}">
      ...
      <div class="p-card__img">
        <img src="${ssImg(product)}" alt="${product.name || "Product"}" loading="lazy" onclick="location.href='/templates/product-detail.html?id=${productId}'">
        ${overlay}
      </div>
      <div class="p-card__body">
